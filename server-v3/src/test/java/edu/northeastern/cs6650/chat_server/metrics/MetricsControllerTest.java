...
class MetricsControllerTest {
  ...
  void setUp() {
    ...
    when(repo.roomMessagesInTimeRange(anyString(), anyString(), anyString(), anyInt()))
        .thenReturn(List.of());
    when(repo.userMessageHistory(anyString(), anyString(), anyString(), anyInt()))
        .thenReturn(List.of());
  ...
  void getMetrics_noParams_usesAutoSelectedDefaults() {
    controller.getMetrics(null, null, null, null, 10, 10);

    verify(repo).mostActiveRoomId();
    verify(repo).mostActiveUserId();
    verify(repo).minTimestamp();
    verify(repo).maxTimestamp();
    verify(repo).roomMessagesInTimeRange("room-1", "2026-03-01T00:00:00Z", "2026-03-01T01:00:00Z", 10);
    verify(repo).userMessageHistory("user-1", "2026-03-01T00:00:00Z", "2026-03-01T01:00:00Z", 10);
  ...
  void getMetrics_explicitParams_passedThroughToRepo() {
    controller.getMetrics("room-5", "user-99",
        "2026-03-10T08:00:00Z", "2026-03-10T09:00:00Z", 5, 10);

    verify(repo, never()).mostActiveRoomId();
    verify(repo, never()).mostActiveUserId();
    verify(repo).roomMessagesInTimeRange(
        "room-5", "2026-03-10T08:00:00Z", "2026-03-10T09:00:00Z", 10);
    verify(repo).userMessageHistory(
        "user-99", "2026-03-10T08:00:00Z", "2026-03-10T09:00:00Z", 10);
  ...
  void getMetrics_coreQueryInputsEchoesResolvedValues() {
    ResponseEntity<Map<String, Object>> response =
        controller.getMetrics("room-5", "user-99",
            "2026-03-10T08:00:00Z", "2026-03-10T09:00:00Z", 10, 10);
  ...
  void getMetrics_topNPassedToAnalyticsQueries() {
    controller.getMetrics(null, null, null, null, 5, 10);
  ...
  @Test
  void getMetrics_sampleSizePassedToListQueries() {
    controller.getMetrics(null, null, null, null, 10, 5);

    verify(repo).roomMessagesInTimeRange(anyString(), anyString(), anyString(), eq(5));
    verify(repo).userMessageHistory(anyString(), anyString(), anyString(), eq(5));
  }
}
